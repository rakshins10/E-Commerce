...
| **`setForm({ ...form, x })` captures a STALE form** | A value written by an effect between two keystrokes is silently discarded by the next one; the server then rejects an empty Guid | Always the functional updater: `setForm((current) => ({ ...current, x }))`. Angular's FormGroup has no equivalent hazard |
| **One e2e suite can break another through the database** | Creating products broke nine storefront assertions that count the seeded catalogue | A suite that creates data must remove it. Withdrawing is enough here - the row survives, the shop stops seeing it |
