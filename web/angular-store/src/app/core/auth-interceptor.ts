 ...
 * credential leak.
 *
 * Catalog browsing is anonymous, so a missing token is not an error here; the
 * request simply goes without one and the server decides.
 */
export const authInterceptor: HttpInterceptorFn = (request, next) => {
  if (!request.url.startsWith(environment.bffBaseUrl)) {
    return next(request);
  }
